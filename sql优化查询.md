```  
    ...
    with connection.cursor() as c:
        sql = "select c.id, c.target_name, c.name_related, c.lower_limit, c.upper_limit, c.daily_upper_limit, c.scene_name, c.scene_desc, c.guarder, c.guard_field, r.date, r.`value`, r.version, r.commit_number, r.is_lower_limit, r.is_upper_limit, r.is_daily_upper_limit, r.today_trend, r.more from " \
              ...
              "inner join (select * from tbl_perf_result as tpr " \
                  "where date in {} " \
                  "order by target_id, date desc) as r " \
              "on c.id=r.target_id " \
              "order by r.target_id, r.date desc, r.commit_number desc;".format(version_dict[version], date_tuple)
